    ...
    reader = make_reader([
        {
            ...
            "params": {
                "name": "reply",
                "arguments": {"text": "hello orchestrator", "meta": {"progress": 50}},
            },
        },
    ])
    writer = FakeWriter()
...
async def test_initialized_triggers_register_and_stream():
    """After `notifications/initialized`, the channel should register with the hub
    and start pumping SSE events into Claude Code as channel notifications."""
    hub = FakeHub()
    hub.push_event("channel_event", {"content": "hello from orchestrator", "meta": {"from": "orch"}})
    ...
    channel_notif = next(m for m in lines if m.get("method") == "notifications/claude/channel")
...
async def test_permission_response_event_becomes_notification():
    ...
    lines = writer.lines()
    perm_notif = next(
        (m for m in lines if m.get("method") == "notifications/claude/channel/permission"),
        None,
    )

---
name: sync-inbox
description: After a claude-tm restart, catch up on commands that arrived during the restart window. Inspects the worker's own event timeline and compares against conversation context to find inbound commands you haven't acted on yet.
---
...
1. **Call `tm_my_inbox()`** (no arguments needed; it resolves your worker
   name from `TM_WORKER_NAME`).

   ```
   mcp__tubemail__tm_my_inbox(limit=20)
   ```

   Returns a list of recent events on your own timeline, mixed
   inbound/outbound/permission-request/permission-response.

2. **Scan for inbound events you haven't yet acted on.** For each
   `kind=inbound` event:
...
3. **Process unhandled inbound events now.** For any inbound you can't
   confirm you handled, treat it as a fresh work order arriving this turn.
   Run it per your normal channel-event handling.

4. **Prefer false positives over false negatives.** If you can't tell
...
```
/sync-inbox: scanned N events, M inbound, K unhandled. Processing unhandled:
...
```
/sync-inbox: all N recent events already accounted for. No missed work.
```

...
|---|---|
| `/restart` | Cleanly restart this Claude Code session via the manager (manager types `/exit`, then re-execs with `--continue` so conversation context survives). Use after editing CLAUDE.md, MCP config, or skills. Pass `fresh=true` (`meta={"kind": "restart", "fresh": true}` in the `tm_send` payload) to restart WITHOUT `--continue` — the startup sequence's automatic `/rename` fires and the worker re-registers cleanly, AND the manager auto-types `/sync-inbox fresh` once the child's empty prompt is ready so the fresh session catches up on any missed timeline events. One-shot; crash-recovery restarts revert to `--continue` and skip the auto-catchup. |
| `/sync-inbox` | After `/restart`, catch up on inbound tubemail events that arrived during the restart window (the SSE subscription was briefly down and doesn't replay). Invoked as `/sync-inbox fresh` when the session has no conversation context: the reconcile then stops at the newest `tm_session_boundary` marker instead of comparing against memory it does not have. |
